...
//**목표:**SmoothDamp를 활용한 3인칭 카메라 팔로우 + 줌 인/아웃 구현

//**요구사항:**
//1.마우스 휠 입력으로 줌 거리를 조절하고 min/max 범위로 클램프
//2. `Mathf.SmoothDamp`로 현재 줌 거리를 목표 줌 거리에 부드럽게 수렴
...
public class Assignment_SmoothCamera : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        Vector3 targetPosition = 
            target.position + offset.normalized * currentZoomDistance + Vector3.up * offset.y;
        ...
        //targetZoomDistance -= scrollWheel * zoomSpeed;
        //targetZoomDistance = Mathf.Clamp(targetZoomDistance, minZoomDistance, maxZoomDistance);
        ...
        //Vector3 desiredPosition = target.position + (offset.normalized * currentZoomDistance);
