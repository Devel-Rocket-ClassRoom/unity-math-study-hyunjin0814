...
public class Assignment_PlanetOrbit : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Matrix4x4 planetTRS = Matrix4x4.TRS(orbitCenter, Quaternion.Euler(0, planetAngle, 0), Vector3.one);
        planetWorldPos = planetTRS.MultiplyPoint3x4(new Vector3(planetOrbitRadius, 0f, 0f));
        transform.position = planetWorldPos;

        float satelliteAngle = Time.time * satelliteOrbitSpeed;
        Matrix4x4 satelliteTRS = Matrix4x4.TRS(planetWorldPos, Quaternion.Euler(0, satelliteAngle, 0), Vector3.one);
        satelliteWorldPos = satelliteTRS.MultiplyPoint3x4(new Vector3(satelliteOrbitRadius, 0f, 0f));
